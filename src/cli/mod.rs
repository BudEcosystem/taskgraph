...
#[command(
    ...
    about = "Task graph primitive for AI agent orchestration.\n\n\
        ...
        \x20 taskgraph task amend         Annotate a future task with new context\n\
    ...
    after_help = "EXAMPLES:\n\
        \x20 taskgraph project create \"auth-system\"                         Create project\n\
...
fn default_db_path() -> String {
    std::env::var("TASKGRAPH_DB").unwrap_or_else(|_| ".taskgraph.db".to_string())
}
...
pub enum Commands {
    ...
    #[command(
        about = "Show project progress: done/total, ready tasks, running agents.\n\n\
                  Three detail levels:\n\
                  \x20 taskgraph status             One-line summary with counts\n\
                  \x20 taskgraph status --detail     Per-task breakdown with status icons\n\
                  \x20 taskgraph status --full       All tasks + dependency edges"
    )]
    Status {
        #[arg(long, help = "Project ID (uses default if not set)")]
        project: Option<String>,
    ...
    Add(CreateTaskArgs),
    #[command(about = "Show full details of a task (shortcut for 'taskgraph task get')")]
    Show(GetTaskArgs),
    ...
    #[command(
        about = "Create a project and set as default (shortcut for 'taskgraph project create')"
    )]
    Init {
...
pub fn run(db: &Database, command: Commands, json: bool, compact: bool) -> Result<()> {
    match command {
        ...
        Commands::Init { name, description } => {
            let project = crate::db::create_project(db, &name, description, None, None)?;
            ...
            if json {
                ...
                if !compact {
                    eprintln!();
                    eprintln!("next: taskgraph add --title \"First task\"");
...
pub fn resolve_project_id(db: &Database, explicit: Option<&str>) -> Result<String> {
    ...
    Err(anyhow!(
        "No project specified. Use --project or run 'taskgraph use <project_id>'."
    ))
}
