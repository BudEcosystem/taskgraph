...
#[command(about = "Manage tasks within a project.\n\n\
              ...
              \x20 taskgraph go --agent NAME        Claim + start next ready task\n\
              \x20 taskgraph done ID --next --agent  Complete current + claim next\n\n\
              PLAN ADAPTATION (mid-flight changes):\n\
              \x20 taskgraph task insert    Add a step between existing tasks\n\
              \x20 taskgraph task amend     Prepend context to a future task's description\n\
              \x20 taskgraph task pivot     Replace a subtree with new tasks\n\
              \x20 taskgraph task split     Break one task into multiple sub-tasks\n\
              \x20 taskgraph task decompose Break a task into subtasks from a YAML file\n\
              \x20 taskgraph task replan    Cancel pending subtasks and create new ones from YAML")]
...
#[command(about = "Preview effects of mutations without applying them.\n\n\
              ...
              EXAMPLES:\n\
              \x20 taskgraph what-if cancel t-a1b2c3\n\
              \x20 taskgraph what-if insert --after t-a1 --before t-b2 --title \"Add auth\"")]
...
pub struct CreateTaskArgs {
    #[arg(long, help = "Project ID (uses default if set via 'taskgraph use')")]
...
pub fn done_cmd(db: &Database, args: DoneArgs, json: bool, compact: bool) -> Result<()> {
    ...
    if !result_provided && has_downstream {
        eprintln!(
            "hint: this task has downstream dependents. Consider: taskgraph done {} --result '<your findings>'",
            task.id
        );
    }
    ...
    if json {
        ...
        if !has_downstream && !compact {
            ...
            eprintln!("      taskgraph add --title \"...\" --dep {}", task.id);
        }
    }
    Ok(())
}
...
fn enrich_transition_error(
    ...
    let Some(TaskgraphError::InvalidTransition(_)) = err.downcast_ref::<TaskgraphError>() else {
        return err;
    };

    let Ok(task) = get_task(db, task_id) else {
        return anyhow!(
            "cannot {action} task {task_id}: task not found. Try `taskgraph show {task_id}` to verify the id"
        );
    };
    ...
    let suggestion = match (action, &task.status) {
        ("start", TaskStatus::Ready) => {
            format!(
                "`taskgraph task claim {} --agent <agent>` then `taskgraph task start {}`",
                task.id, task.id
            )
        }
        ("start", TaskStatus::Pending) => {
            format!(
                "task is blocked by dependencies; inspect with `taskgraph show {}`",
                task.id
            )
        }
        ("start", TaskStatus::Running) => {
            format!(
                "task is already running; continue work or run `taskgraph done {}`",
                task.id
            )
        }
        ("complete", TaskStatus::Claimed) => {
            format!(
                "run `taskgraph task start {}` first, then `taskgraph done {}`",
                task.id, task.id
            )
        }
        ("complete", TaskStatus::Ready) => {
            format!(
                "run `taskgraph task claim {} --agent <agent>` and `taskgraph task start {}` first",
                task.id, task.id
            )
        }
        ...
        ("complete", TaskStatus::Done | TaskStatus::DonePartial) => {
            format!(
                "task is already completed; inspect with `taskgraph show {}`",
                task.id
            )
        }
        ("fail", TaskStatus::Claimed) => {
            format!(
                "run `taskgraph task start {}` first, then `taskgraph task fail {} --error <msg>`",
                task.id, task.id
            )
        }
        ...
        _ => format!("inspect task state with `taskgraph show {}`", task.id),
    };
