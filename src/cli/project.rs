...
#[command(about = "Manage projects.\n\n\
              ...
              The first project created is automatically set as the default (see 'taskgraph use').")]
...
pub fn run(db: &Database, command: ProjectCommand, json: bool, compact: bool) -> Result<()> {
    match command.command {
        ProjectSubcommand::Create(args) => {
            let project = crate::db::create_project(db, &args.name, args.description, None, None)?;
        ...
        ProjectSubcommand::List(args) => {
            let mut projects = list_projects(db, None)?;
